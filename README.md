...
A deterministic benchmark for evaluating AI agents on document-heavy professional workflows. The suite generates realistic office workloads (xlsx, docx, pdf, csv) organized into **scenario packs** — self-contained bundles of test cases, formatters, and gold standards that share a single canonical data model. Each pack simulates a fictional company ecosystem with cross-referential integrity across all deliverables.

The default pack — Cascade Industries (Audit, Tax, Advisory) — generates 18 test cases; additional packs cover Legal/HR diligence (TC-19 through TC-21), European IFRS/OECD variants (10 cases), and Swiss jurisdiction variants (TC-23). The full suite contains 32 test cases across 4 scenario packs.
...
| [`docs/european-variants.md`](./docs/european-variants.md) | European/IFRS/OECD variant pack: affected TCs, jurisdictions, standards, scoring |
...
| [`specs/european-professional-services-variants.md`](./specs/european-professional-services-variants.md) | European variants spec with full bead backlog |
| [`specs/european-standards-reference.md`](./specs/european-standards-reference.md) | Authoritative standards and guidance for European variants |
...
|---|---|---|---|
| `cascade_accounting_core` | Cascade Accounting Core | TC-01 through TC-18 (18 cases) | — |
| `cascade_europe_ifrs` | Cascade Europe -- IFRS/OECD Variants | TC-04-EU through TC-18-EU (10 cases) | `cascade_accounting_core` |
| `cascade_legal_hr_diligence` | Cascade Legal/HR Diligence | TC-19 through TC-21 (3 cases) | `cascade_accounting_core` |
| `ch_variants` | Swiss Variants (CH) | TC-23 (1 case) | `cascade_accounting_core` |
...
### European IFRS/OECD Variants (`cascade_europe_ifrs`)

| ID | US Original | Service Line | Description |
|----|-------------|-------------|-------------|
| TC-04-EU | TC-04 | Audit | IFRS 16 lease extraction |
| TC-06-EU | TC-06 | Tax | IAS 12 income tax provision (multi-jurisdiction, Pillar Two judgment trap) |
| TC-07-EU | TC-07 | Tax | European partnership/investment reporting (multi-currency) |
| TC-08-EU | TC-08 | Tax | R&D incentive study (French CIR + German Forschungszulage) |
| TC-09-EU | TC-09 | Tax | OECD transfer pricing documentation (master/local file) |
| TC-10-EU | TC-10 | Tax | EU VAT and cross-border tax positions |
| TC-12-EU | TC-12 | Advisory | European diligence data room (GDPR, works council, EU corporate forms) |
| TC-15-EU | TC-15 | Advisory | European multi-jurisdiction DCF valuation under IFRS |
| TC-16-EU | TC-16 | Cross-service | European engagement letter (ISA framing, VAT scope) |
| TC-18-EU | TC-18 | Cross-service | IFRS/ISA workpaper rollforward |

### Swiss Variants (`ch_variants`)

| ID | Service Line | Difficulty | Description |
|----|-------------|------------|-------------|
| TC-23 | Audit | Complex | Swiss multi-currency bank reconciliation (CHF/EUR/USD) |

Swiss variants are not generated by default. Select the pack explicitly:

```bash
uv run python generate_test_suite.py \
    --packs cascade_accounting_core ch_variants \
    --output /tmp/test_suite
```

### European IFRS/OECD Variants

EU variants are not generated by default. Select the pack explicitly:
...
See [`docs/european-variants.md`](./docs/european-variants.md) for jurisdiction profiles, source standards, and design rationale.
